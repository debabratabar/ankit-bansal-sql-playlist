...
( 
with temp1 as (
select * , dense_rank() over(partition by user_id order by created_at)  rn 
from marketing_campaign
--where user_id in ( 11,14,25)
) , temp2 as (
select user_id , string_agg(product_id::varchar(5) , ',' order by product_id)  tot_prod
...
select t2.user_id , t2.tot_prod , t3.tot_prod
from temp2 t2  join temp3 t3 
on t2.user_id = t3.user_id
where t2.tot_prod != t3.tot_prod
) b 
on a.user_id = b.user_id 



select *  , dense_rank() over(partition by user_id order by created_at ) 
