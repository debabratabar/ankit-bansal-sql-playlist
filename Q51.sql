...
select 
ut.Segment , count(distinct ut.user_id  ) total_user_count , count( distinct bt.user_id)  user_booked_flight_on_apr_2022
from user_table ut  left join (select user_id from booking_table   where to_char(booking_date , 'yyyy-MM') = '2022-04' and line_of_business='Flight' ) bt
...
select  * from (
select user_id , booking_date , line_of_business , rank() over(partition by user_id order by booking_date  ) rn
 from booking_table
)a  where rn =1 and line_of_business ='Hotel'

--Q3



select user_id , max(booking_date) - min(booking_date)  from  booking_table
group by user_id
order by user_id
--Q4

select 
ut.Segment , count(  case when bt.line_of_business ='Hotel' then bt.user_id  end ) total_Hotel_count 
, count(  case when bt.line_of_business ='Flight' then bt.user_id  end ) total_Flight_count
from user_table ut   join (select user_id , line_of_business from booking_table   where extract(year from booking_date) = 2022 ) bt
on ut.User_id=bt.user_id
group by ut.Segment
